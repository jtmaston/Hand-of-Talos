...
void MainWindow::updateAxisDisplay() // this updates the axes display
{
    ...
    ui->a1_d->setText(std::string(std::string("Axis 1: ") + std::to_string(dev_.currentPosition_[0]).substr(0, 5) + "°").c_str()); // and set the strings for
    ui->a2_d->setText(std::string(std::string("Axis 2: ") + std::to_string(dev_.currentPosition_[1]).substr(0, 5) + "°").c_str()); // the labels
    ui->a3_d->setText(std::string(std::string("Axis 3: ") + std::to_string(dev_.currentPosition_[2]).substr(0, 5) + "°").c_str());
    ui->a4_d->setText(std::string(std::string("Axis 4: ") + std::to_string(dev_.currentPosition_[3]).substr(0, 5) + "°").c_str());
    ui->a5_d->setText(std::string(std::string("Axis 5: ") + std::to_string(dev_.currentPosition_[4]).substr(0, 5) + "°").c_str());
    ui->a6_d->setText(std::string("Gripper: " + std::to_string(int(dev_.currentPosition_[5])) + "%").c_str());
